					...
					<div className='grid grid-cols-2 gap-4 mt-6'>
						{filteredRecipes.map(recipe => (
							<div key={recipe.id} className='bg-white rounded-xl overflow-hidden relative border border-neutral-100 shadow-sm'>
								<img src={recipe.image.thumbnail} alt={recipe.name} className='w-full h-32 object-cover rounded-t-xl' />
								<div className='p-3'>
								...
								<button
									...
									className='absolute bottom-3 right-3 bg-green-400 hover:bg-green-500 text-white rounded-full w-6 h-6 flex items-center justify-center transition-colors duration-200'
								>
